/*
 ...
 Args (sizes):
   ...
   double *xs: array of grid points, including left border (sum(n+1))
   lapackint *ttrank: tt ranks (d+1)
   double *ttcore: TT2.0 contigous storage of tt cores (sum(r*n*r))
   lapackint M: number of points
   double *q: seed points (M*d)
   double *z: transformed points (M*d) !should be allocated
   double *Pz: sampling density values at z (M) !should be allocated
*/
...
#define lapackint int   // for Python
...
void tt_irt1(lapackint d, lapackint *n, double *xs, lapackint *ttrank, double *ttcore, lapackint M, double *q, double *z, double *Pz)
{
  ...
  else fk = (double *)malloc(sizeof(double)*rmax*(nmax+1));
  fk2 = (double *)malloc(sizeof(double)*Mb*(nmax+1));
  ...
  for (k=d-1; k>0; k--) {
    ...
    dgemm_(&cN, &cN, &i, &ione, &ttrank[k+1], &done, &ttcore[pstt[k]], &i, C[k], &ttrank[k+1], &dzero, &fk[ttrank[k]], &i);
    // grid steps
    for (i=0; i<n[k]; i++) h[i] = xs[psxs[k]+i+1] - xs[psxs[k]+i];
    // extrapolate fk to xs[k][0]
    C2 = (xs[psxs[k]]-xs[psxs[k]+1])/h[1];
    C1 = (xs[psxs[k]+2]-xs[psxs[k]])/h[1];
    dcopy_(&ttrank[k], &fk[ttrank[k]], &ione, &fk[0], &ione);
    dscal_(&ttrank[k], &C1, &fk[0], &ione);
    daxpy_(&ttrank[k], &C2, &fk[ttrank[k]*2], &ione, &fk[0], &ione);
    // integrate
  ...
  for (m=0; m<M; m+=Mb) {
    ...
    for (k=0; k<d; k++) {
      ...
      i=n[k]+1;
      dgemm_(&cN, &cN, &Mb, &i, &ttrank[k], &done, fkm1, &Mb, fk, &ttrank[k], &dzero, fk2, &Mb);
      // make this nonnegative
      for (j=0; j<(n[k]+1)*Mb; j++) fk2[j]=fabs(fk2[j]);
      ...
      for (i=0; i<Mb; i++) {
        ...
        Pz[m+i] *= fk2[i+i0*Mb]*C1 + fk2[i+(i0+1)*Mb]*C2;

        if (k<d-1) {
          // sample the k-th block onto xk
          if (i0>0) {
            for (j=0; j<ttrank[k+1]; j++) {
              dcopy_(&ttrank[k], &ttcore[pstt[k]+(i0-1)*ttrank[k]+j*ttrank[k]*n[k]], &ione, &fkm2[j*ttrank[k]], &ione);
              dscal_(&ttrank[k], &C1, &fkm2[j*ttrank[k]], &ione);
              daxpy_(&ttrank[k], &C2, &ttcore[pstt[k]+i0*ttrank[k]+j*ttrank[k]*n[k]], &ione, &fkm2[j*ttrank[k]], &ione);
            }
          } else {
            C1 = C1*(xs[psxs[k]+2]-xs[psxs[k]])/h[1] + C2;
            C2 = ((x2-xk)/hq)*(xs[psxs[k]]-xs[psxs[k]+1])/h[1];
            for (j=0; j<ttrank[k+1]; j++) {
              dcopy_(&ttrank[k], &ttcore[pstt[k]+j*ttrank[k]*n[k]], &ione, &fkm2[j*ttrank[k]], &ione);
              dscal_(&ttrank[k], &C1, &fkm2[j*ttrank[k]], &ione);
              daxpy_(&ttrank[k], &C2, &ttcore[pstt[k]+ttrank[k]+j*ttrank[k]*n[k]], &ione, &fkm2[j*ttrank[k]], &ione);
            }
          }
