function [q, lFapp]=tt_rt_sqr(xsf, f, x)
% Forward CDF (Rosenblatt) transform through the square root of the density F
% This version computes CDFs in each variable via quadratic splines
% Inputs (sizes):
...
%   x: samples in the range defined by xsf (an M x D matrix). To sample
%      from marginal, 0 < D <= d is allowed.
%
% Outputs (sizes):
%   q: samples mapped from x via the CDF (M x D matrix)
%   lFapp: log(approximate PDF) (inv. Jacobian of x) sampled on x
%
% See also: tt_irt_sqr, tt_irt_lin, tt_irt_fourier
...
for i_block=1:num_blocks
    ...
    for k=1:min(d,size(x,2))    % Simple trick to sample marginal if needed
        ...
        %%% Interpolate the marginal CDF Ck
        % Binary search for the closest to x points
        xk = x(start_pos:(start_pos+Mb-1),k);
        i0 = ones(Mb,1);
        i2 = n(k)*ones(Mb,1);
        while (any((i2-i0)>1))
            i1 = floor((i0+i2)/2);
            x1 = xsf(pos(k)+i1-1);  % grid points at i0
            ind_left = xk>x1; % at these indices i0 becomes i1
        ...
        % Interpolate CDF via quadratic spline
        Aq = 0.5*(f2-f1)./h3;
        qk = Aq.*(xk-x1).^2 + f1.*(xk-x1) + C1;
                        
        q(start_pos:(start_pos+Mb-1),k) = qk;
