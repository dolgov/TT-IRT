...
if (~isfield(params, 'corr_length'))
    params.corr_length = input('Prior model correlation length = ? (default 1): ');
...
if (~isfield(params, 'meshlevel'))
    params.meshlevel = input('Spatial mesh level = ? ("1" <=> h=1/32) (default 2): ');
...
if (~isfield(params, 'sigma_n'))
    params.sigma_n = input('Noise variance = ? (default 1e-2): ');
    if (isempty(params.sigma_n))
        params.sigma_n = 1e-2;
    end
end
if (~isfield(params, 'm0'))
    params.m0 = input('Number of measurements in each direction (sqrt(m0)) = ? (default 3): ');
    if (isempty(params.m0))
        params.m0 = 3;
    end
end
if (~isfield(params, 'y0'))
    params.y0 = input('Truth value of parameters for synthetic data = ? (default 1.5): ');
...
if (~isfield(params, 'log2N'))
    params.log2N = input('log2(number of samples in the chain) = ? (default 14): ');
